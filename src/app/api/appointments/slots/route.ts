...
export async function GET(req: Request) {
  try {
    ...
    // These four reads do not depend on one another, and this endpoint is hit
    // every time the booking flow changes date or provider. Run as four
    // sequential awaits it paid four full round trips to a database in another
    // region before it could answer. Validation below still runs in the same
    // order, so the same request still produces the same error and status.
    const [provider, setting, service, booked] = await Promise.all([
      db.user.findFirst({
